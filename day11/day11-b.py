...
class SeatingSystemAdvanced(SeatingSystem):
    adjacency = [
        complex(i, j)
        for i in range(-1, 2)
        for j in range(-1, 2)
    ...
    def next_seat_position(self, seat):
        ...
        if self.prior[seat] == "#":
            n_occupied = 0
            for adj in self.adjacency:
                if self._get_dist_val(seat, adj) == "#":
                    n_occupied += 1
                    if n_occupied >= 5:
                        return "L"
            return "#"
    ...
    def _get_dist_val(self, seat, direction):
        n = 1
        while self.prior.get(seat + direction * n, "L") == ".":
            n += 1
        return self.prior.get(seat + direction * n, "L")


def part_b(_file_location):
    ss = SeatingSystemAdvanced.from_file(_file_location)
